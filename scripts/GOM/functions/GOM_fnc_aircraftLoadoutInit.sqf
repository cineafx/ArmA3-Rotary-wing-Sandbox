...
GOM_fnc_refuelCheck = {
	...
	_vehs = (_veh nearEntities ["All",50]) select {((speed _x) < 1) && {alive _x} && {(_x getvariable ["GOM_fnc_fuelCargo",0]) > 0}};
...
GOM_fuelLeak = {
	...
	_hitpoints apply {
		_count = _count + 1;
		if (toUpper _x find "FUEL" >=0 && _damages select _count > 0) then {
...
GOM_fnc_landingsCheck = {
    ...
    waituntil {
        sleep 3;
    	if ((alive _plane) && {istouchingground _plane} && {(speed _plane) < 100}) then {_check = _check + 1} else {_check = 0};
...
//run on server only, init for supply vehicles, only sets variables and fuel/ammo/repaircargo to 0
	...
	while {true} do {
		...
		_refuelVehs = vehicles select {((_x getVariable ["GOM_fnc_fuelCargo",-1]) < 0) && {alive _x} && {(getNumber (configfile >> "CfgVehicles" >> typeof _x >> "transportFuel")) > 0}};
		...
		_repairVehs = vehicles select {((_x getVariable ["GOM_fnc_repairCargo",-1]) < 0) && {alive _x} && {(getNumber (configfile >> "CfgVehicles" >> typeof _x >> "transportRepair")) > 0}};
	...
	if (GOM_fnc_aircraftLoadout_NeedsAmmoSource) then {
		...
		_reammoVehs = vehicles select {((_x getVariable ["GOM_fnc_ammoCargo",-1]) < 0) && {alive _x} && {(getNumber (configfile >> "CfgVehicles" >> typeof _x >> "transportAmmo")) > 0}};
...
GOM_fnc_showResourceDisplay = {
	...
	if (GOM_fnc_aircraftLoadout_NeedsAmmoSource) then {
		...
		} foreach ((player nearEntities ["All",50]) select {((speed _x) < 15) && {alive _x} && {(_x getvariable ["GOM_fnc_ammocargo",-1]) >= 0}});
	...
	if (GOM_fnc_aircraftLoadout_NeedsFuelSource) then {
		...
		} foreach ((player nearEntities ["All",50]) select {((speed _x) < 15) && {alive _x} && {(_x getvariable ["GOM_fnc_fuelcargo",-1]) >= 0}});
	...
	if (GOM_fnc_aircraftLoadout_NeedsRepairSource) then {
		...
		} foreach ((player nearEntities ["All",50]) select {((speed _x) < 15) && {alive _x} && {(_x getvariable ["GOM_fnc_repaircargo",-1]) >= 0}});
